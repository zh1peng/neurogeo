test_that("NIfTI golden input preserves affine, frames, indices, and sidecar", {
  ...
  expect_equal(x$base$geometry$dim, c(2L, 2L, 2L))
  expect_equal(x$base$geometry$affine, expected_affine)
  expect_equal(x$base$geometry$header_transforms$qform, expected_qform)
  expect_equal(x$base$geometry$header_transforms$sform, expected_affine)
  expect_identical(x$base$geometry$header_transforms$active, "sform")
  expect_equal(x$base$geometry$source_voxel_index[1, ], c(0L, 0L, 0L))
  expected_values <- matrix(1:16, nrow = 8)
  colnames(expected_values) <- c("frame_1", "frame_2")
  expect_equal(x$values, expected_values)
  expect_equal(x$layers$source_frame, 0:1)
  expect_equal(x$history$bids_sidecar$RepetitionTime, 2)
  ...
  expect_equal(nrow(metadata_only$layers), 2L)
  expect_equal(nrow(metadata_only$base$elements), 8L)
})
...
test_that("GIFTI golden geometry, distance_method, and labels remain aligned", {
  ...
  expect_equal(nrow(x$base$elements), 4L)
  expect_equal(nrow(x$base$geometry$faces), 4L)
  expect_equal(as.vector(x$values), c(1.5, 2.5, 3.5, 4.5))
  expect_identical(x$base$geometry$face_source_index_base, 0L)
  expect_equal(x$base$labels$atlas$values, c(0L, 3937500L, 3937500L, 0L))
...
test_that("FreeSurfer golden surface, curv, annot, and MGH read without binaries", {
  ...
  expect_identical(surface$base$geometry$face_source_index_base, 0L)
  expect_equal(length(surface$base$labels$annot$values), 4L)
  ...
  expect_equal(volume$base$geometry$dim, c(2L, 2L, 2L))
...
test_that("unified reader dispatches standard file suffixes", {
  ...
  expect_s3_class(cifti, "ngeo_grayordinate")
})
...
test_that("CIFTI golden dscalar reconstructs surface-volume ordering", {
  ...
  expect_equal(
    x$base$geometry$components$thalamus_left$voxel_index,
...
test_that("CIFTI golden dlabel and dtseries preserve map semantics", {
  ...
  expect_identical(labels$measures$support_behavior, "categorical")
  expect_identical(labels$measures$aggregation, "mode")
  expect_equal(labels$base$labels$atlas$table$Key, c(0, 1))
  ...
  expect_equal(series$layers$time, c(0, 0.8, 1.6))
  expect_identical(series$layers$time_unit, rep("SECOND", 3L))
})
