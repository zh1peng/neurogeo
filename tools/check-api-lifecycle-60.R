...
if (!identical(sum(expected$type == "export"), 248L)) {
  stop("The registered export count differs from the approved 6.1 ADR.")
}
if (!identical(sum(expected$type == "s3_method"), 99L)) {
  stop("The registered S3 method count differs from the approved 6.1 ADR.")
}
