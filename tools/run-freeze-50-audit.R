...
output <- if (length(args)) args[[1L]] else
  file.path("check-output", "freeze-50-audit.json")
if (!requireNamespace("jsonlite", quietly = TRUE)) {
  stop("The 5.0 freeze audit requires jsonlite.")
}
...
checks <- list(
  ...
  specifications_present = all(file.exists(file.path(
    "inst", "spec", c("API-5.0.md", "NGCS-5.0.md", "migration-5.0.md")
  )))
)
pass <- all(unlist(checks, use.names = FALSE))
report <- list(
  schema = "neurogeo/api-freeze-5.0",
...
if (!pass) stop("The 5.0 API freeze audit failed.", call. = FALSE)
