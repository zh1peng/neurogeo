...
surface_changed <- aggregate_to(surface, surface, surface_map)
...
gray_map <- ngeo_atlas_map(
  grayordinates,
  grayordinates$base$elements$component_id
)
gray_changed <- aggregate_to(
  grayordinates,
  gray_map$target,
  gray_map
)
...
result <- list(
  ...
  workflows = list(
    gifti_registered_surface = list(
      source_elements = nrow(surface$base$elements),
      target_elements = nrow(surface$base$elements),
    ...
    nifti_label_segmentation = list(
      source_elements = nrow(volume$base$elements),
      regions = nrow(volume_changed$base$elements),
    ...
    cifti_hybrid_atlas = list(
      source_elements = nrow(grayordinates$base$elements),
      components = nrow(gray_changed$base$elements),
