test_that("surface subset synchronizes values, IDs, coordinates, and faces", {
  ...
  old_ids <- base_elements(x)$element_id
  y <- ngeo_subset(x, elements = c(4, 1, 2), layers = "second")

  expect_equal(as.vector(values(y)), c(24, 21, 22))
  expect_identical(base_elements(y)$element_id, old_ids[c(4, 1, 2)])
  expect_equal(y$base$geometry$coordinates$active, coordinates[c(4, 1, 2), ])
  expect_equal(y$base$geometry$faces, matrix(c(2, 3, 1), nrow = 1))
  expect_identical(layers(y)$name, "second")
  expect_silent(ngeo_validate(y, "strict"))
  expect_false(identical(base_hash(x), base_hash(y)))
  expect_identical(
    tail(history(y)$operations, 1)[[1]]$operation,
    "ngeo_subset"
  )
})
...
test_that("volume subset rebuilds the active lattice mask", {
  ...
  expect_equal(sum(y$base$geometry$mask), 3L)
  expect_equal(nrow(y$base$geometry$voxel_index), 3L)
...
test_that("grayordinate subset rebuilds component global rows", {
  ...
  expect_equal(y$base$geometry$components$left$global_rows, 2L)
  expect_equal(y$base$geometry$components$volume$global_rows, c(1L, 3L))
  expect_silent(ngeo_validate(y, "strict"))
})
...
test_that("region subset trims adjacency and drops excluded memberships", {
  ...
  expect_equal(dim(y$base$topology$adjacency), c(2L, 2L))
  expect_identical(y$base$geometry$membership, c("A", NA, "C", "A"))
  expect_equal(as.vector(y$values), c(3, 1))
  expect_silent(ngeo_validate(y, "strict"))
})

test_that("subset rejects unknown or duplicated selections", {
  x <- ngeo_point(matrix(c(0, 0, 1, 1), ncol = 2, byrow = TRUE))
