...
manifest_path <- file.path(
  "inst", "extdata", "reference-4.3.1", "manifest.csv"
)
if (!file.exists(manifest_path)) {
  stop("The 4.3.1 flatmap reference-data manifest is missing.")
}
...
if (any(!required %in% names(manifest)) ||
  ...
  stop("The 4.3.1 flatmap reference-data manifest is invalid.")
}
...
for (i in seq_len(nrow(manifest))) {
  ...
  if (!verify(output, row)) {
    temporary <- tempfile(".reference-431-", tmpdir = cache)
